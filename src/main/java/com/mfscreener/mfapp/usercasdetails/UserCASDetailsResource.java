...
@RestController
@RequestMapping(value = "/api/userCASDetails", produces = MediaType.APPLICATION_JSON_VALUE)
public class UserCASDetailsResource {

    private final UserCASDetailsService userCASDetailsService;
    private final UserInfoRepository userInfoRepository;

    public UserCASDetailsResource(
            final UserCASDetailsService userCASDetailsService, final UserInfoRepository userInfoRepository) {
        this.userCASDetailsService = userCASDetailsService;
        this.userInfoRepository = userInfoRepository;
    }

    @GetMapping
    public ResponseEntity<List<UserCASDetailsDTO>> getAllUserCASDetails() {
        return ResponseEntity.ok(userCASDetailsService.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<UserCASDetailsDTO> getUserCASDetails(@PathVariable(name = "id") final Long id) {
        return ResponseEntity.ok(userCASDetailsService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<Long> createUserCASDetails(@RequestBody @Valid final UserCASDetailsDTO userCASDetailsDTO) {
        final Long createdId = userCASDetailsService.create(userCASDetailsDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Long> updateUserCASDetails(
            @PathVariable(name = "id") final Long id, @RequestBody @Valid final UserCASDetailsDTO userCASDetailsDTO) {
    ...
    public ResponseEntity<Map<Long, Long>> getUserInfoValues() {
        return ResponseEntity.ok(userInfoRepository.findAll(Sort.by("id")).stream()
